...
export const PurchaseOrderList = () => {
  ...
  const { purchaseOrderList, getPurchaseOrderList } =
  ...
  useEffect(() => {
    settempAllPurchaseOrder(purchaseOrderList);
  }, [purchaseOrderList]);

  console.log(tempAllPurchaseOrder);
  ...
  return (
    <>
      ...
      <div className="p-6 md:px-4 xl:px-6 2xl:px-8 h-[calc(100dvh-80px)] min-h-[400px] flex flex-col">
          ...
          <div className=" grid grid-cols-12 gap-3 lg:text-sm text-xs xl:text-base">
            <button
              ...
              onClick={(e) => {
                ...
                // Filter purchase orders where invoice_url is "n/a"
                const expandedPurchase = tempAllPurchaseOrder
                  .filter(
                    (purchase) =>
                  ...
                  .flatMap((purchase) =>
                    purchase.list_items.map((item) => ({
                      "Purchase Order ID": purchase.po_id,
                      "Vendor Name": purchase.vendor_name,
                      Email: purchase.email,
                      Item: item.item_description,
                      Quantity: item.quantity,
                      Amount: item.gross_amount,
                      Date: purchase.po_date,
                    }))
                  );

                // Check if we have any valid data
                if (!expandedPurchase.length) {
                  // showToast("No matching purchase orders found.");
                  return;
                }

                console.log(expandedPurchase);

                try {
                  exportToExcel(expandedPurchase, "Purchase-order-list.xlsx");
                } catch (err) {
                  console.error("Export failed:", err);
                  showToast(
                    "Failed to export purchase orders. Please try again."
                  );
                }
              }}
            ...
            <FilterPurchaseButton setdata={settempAllPurchaseOrder} />
            <button
              aria-label="Add Purchase"
              onClick={() => {
                navigate(`/purchase/createOrder/new`);
              }}
        ...
        {!isLoading && tempAllPurchaseOrder && (
          <ShowPurchaseOrderInTable allPurchase={tempAllPurchaseOrder} />
        )}
      </div>
    </>
  );
...
const FilterPurchaseButton = ({ setdata = () => {} }) => {
  ...
  useEffect(() => {
    ...
    const allItemNames = purchaseOrderList.flatMap((obj) =>
...
const FilterPurchasedata = ({
  ...
  const { handelMultipleFilter, purchaseOrderList } =
    useContext(PurchaseOrderContext);

  useEffect(() => {
    const handelIsResetFilters = async () => {
      setdata(purchaseOrderList);
...
const SearchPurchaseOrderComponent = ({ setData }) => {
  const [query, setquery] = useState(null);
  const [isSeachNow, setisSeachNow] = useState(false);
  const { searchPurchaseOrder, purchaseOrderList } =
    useContext(PurchaseOrderContext);

  const handelSearchPurchaseOrder = async () => {
    console.log("searching");
    if (!query) setData(purchaseOrderList);

    const searchData = await searchPurchaseOrder(query);
    setData(searchData);
    setisSeachNow(false);
  };

  useEffect(() => {
    isSeachNow && handelSearchPurchaseOrder();
  }, [isSeachNow]);
